...
const BookSection: React.FC<UIBookSection> = ({ sectionTitle }) =>  {
    ...
    useEffect(() => {
        document.documentElement.style.setProperty("--qtd", qtd);
        document.documentElement.style.setProperty("--margin", `${marginCards}px`);
    }, [qtd, marginCards]);

    const moveLeft = () => {
        if (!slideClass)
            setSlideClass("animating-right");
    };
    
    const moveRight = () => {
        if (!slideClass)
            setSlideClass("animating-left");
    };
    ...
    return (
        <BookSectionContainer>
            ...
            <CaroselContainer>
                <NavigationButtons className="left-btn" onClick={moveLeft}><b>{"<"}</b></NavigationButtons>
                ...
                <NavigationButtons className="right-btn" onClick={moveRight}><b>{">"}</b></NavigationButtons>
            </CaroselContainer>
